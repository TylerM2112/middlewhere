...
// *** what is this .props .locations. state????
    console.log("CDMCDMCDMCDMCDM",this.props.location.state)
    
    if(selectedGroups){
      //should post to newevents with selected groups
  ...
  addToSelectedPlaces(id){
    ...
    if(index === -1){
      ...
      //why would we write functionality that changes the color to red when the div is red initially???
      newSelectedPlaces -= 1;
      //decrement selected places 
    }

    this.setState({markers:newMarkers,selectedPlaces:newSelectedPlaces})
    //  pass in markers and new Selectedplaces into state
  }
...
//getting scrolling position
  ...
  addSelectedPlacesToEvent(){
    //takes selected places and adds them to events
